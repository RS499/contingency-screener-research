...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{data/gate_schematic_v2.png}
...
\caption{Escalation (solid, left axis) and missed violations (dotted, right axis) are plotted against the safety target and averaged over five splits. The missed rate first falls below 1\% at 0.94 coverage for ridge and 0.97 for the gradient-boosted model, with high escalation at both.}
\label{fig:tradeoff}
\end{figure}
...
% =====================================================================
% ACKNOWLEDGMENTS -- carried over verbatim from the IEEEtran version.
%
% TWO ITEMS TO RESOLVE BEFORE SUBMISSION:
% (1) AI disclosure. The sentence below says "validate the code and
%     grammar." notes/ai-prompt-log.md records AI-WRITTEN scaffolding
%     for the dataset generator, screener and eval harness, plus text
%     revision. Audit the log and widen this to match, per the STS
%     2027 generative-AI table.
% (2) The GitHub link. STS judges do not follow external links, so the
%     URL answers nothing here; code must live inside the 20 pages or
%     nowhere. The sentence also asserts every number came from
%     committed code -- verify that holds for the 14% bin share, the
%     1.5% figure at L=0.95, and bases_clearing_0p95.json.
% =====================================================================
