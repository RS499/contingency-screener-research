...
The main results of the experiments involve a network with a realistic topology and synthetic loads and generation. This escalation floor is determined by the nature of the distribution of the data relative to the 0.94 pu limit. For instance, in a thermally realistic regeneration of the IEEE 30-bus network, 7.09\% of contingencies reach the threshold level, while another 15.40\% fall below it. For case118, 56.86\% reach the threshold level and 17.48\% fall below it. The original 30-bus system is thermally unrealistic, since it has 111.83\% line loading in its base case. As a result, the gradient-boosted model at a 0.97 coverage target yields 5.84$\pm$1.25\% escalations. The target for which the miss rate becomes less than 1\% remains unresolved after five split-ups: for 0.96 it is 0.91$\pm$0.22\%, with only two of five being under this limit. We use 0.94 pu as a conservative choice. Only 86 of the 1,500 base cases are above 0.95 pu, and increasing the threshold to 0.95 pu leads to a ridge escalation of 1.38$\pm$0.33\%. In other words, it finds everything hazardous.

Our claims of safety are only true for N-1 contingency cases, as the safety claims apply only to the specific set of N-1 branch-contingency cases, since there are roughly one-quarter of converged rows that include a single-generator outage occuring simultaneously with the branch outage, which does not meet the N-1 criterion. In addition, mathematical proof is unable to determine the safety of N-2 outages since conformal prediction requires the data to be exchangeable with the calibration set \cite{tibshirani2019}. Also, we only checked if the final voltage was too low, not for thermal overheating, over-voltage, or whether or not the grid crashed in the first place, which is called dynamic stability. The band only counts instances of where the model is wrong and not how badly it fails. In addition, the worst case, at 0.0915 per unit, is an example of a sudden collapse where a generator at that case's weakest bus hits its reactive limit and can no longer regulate voltage. A model trained only on pre-outage features cannot predict such a situation. Finally, all of the testing data was generated from small variations around one pre-contingency grid setup, so we are unsure of the frequency or timing at which operators should recalibrate the models as seasons and demand change throughout the year.

% =====================================================================
\section{Conclusion and future work}
\label{sec:conclusion}

Our models can predict N-1 under-voltage contingencies on the IEEE 118-bus system. On this network, safety and speed are inversely related, though the 30-bus result shows that this depends on data distribution relative to the limit. At 90\% coverage, the models are fast but miss too many contingencies, and to bring the missed rate close to 1\% on average requires escalating around two-thirds of cases, with error bars that still touch the 1\% threshold. The speedup on this network decreases enough to be comparable to the speed of the solver itself. The main reason is that most of the cases fall very close to the limit. As a result, the bands often straddle this limit and then must escalate to the solver as the result is uncertain. Our contribution is explaining why that floor exists, and a note of caution to anyone expecting large speedups from a surrogate model calibrated to be safe on average over the N-1 distribution. Future work includes testing multiple-element outages and a third network to see whether the speedup achieved correlates with how many contingencies lie near the limit.
